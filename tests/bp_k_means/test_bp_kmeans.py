...
from bp_k_means.algos.bp_kmeans import InitStrategy, RankingMetric, bp_kmeans
...
class TestMetrics:
    """Test every supported label-selection metric."""

    def test_names_match_paper(self) -> None:
        """Expose the metric names used in the paper."""
        assert [ranking_metric.name for ranking_metric in RankingMetric] == [
            "M_L",
            "M_C",
            "M_ERL",
            "M_RL",
    ...
    def test_all_metrics_produce_valid_output(
        self, sample_data: tuple[np.ndarray, np.ndarray], ranking_metric: RankingMetric
    ) -> None:
...
class TestAllCombinations:
    ...
    @pytest.mark.parametrize("ranking_metric", list(RankingMetric))
    @pytest.mark.parametrize("init", list(InitStrategy))
    def test_all_metric_init_combinations(
        self,
...
class TestEdgeCases:
    ...
    def test_single_label(self) -> None:
        """Split a single label into the requested number of clusters."""
        rng = np.random.default_rng(42)
